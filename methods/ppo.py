...
class PPO():
  def __init__(self):
    self.StepData = collections.namedtuple(
          'StepData',
          ('observation', 'logits', 'action', 'reward', 'done', 'truncation'))


  def sd_map(self, f: Callable[..., torch.Tensor], *sds):
    """Map a function over each field in StepData."""
    items = {}
    keys = sds[0]._asdict().keys()
    for k in keys:
      items[k] = f(*[sd._asdict()[k] for sd in sds])
    return self.StepData(**items)


  def eval_unroll(self, agent, env, length):
  ...
  def train_unroll(self, agent, env, observation, num_unrolls, unroll_length):
    """Return step data over multple unrolls."""
    sd = self.StepData([], [], [], [], [], [])
    for _ in range(num_unrolls):
      one_unroll = self.StepData([observation], [], [], [], [], [])
      ...
      one_unroll = self.sd_map(torch.stack, one_unroll)
      sd = self.sd_map(lambda x, y: x + [y], sd, one_unroll)
    td = self.sd_map(torch.stack, sd)
  ...
  def train(self, 
    ...
    self.agent = Agent(policy_layers=policy_layers,
                ...
                device=device)            
    self.agent = torch.jit.script(self.agent.to(device))
    ...
    for eval_i in range(eval_frequency + 1):
      if progress_fn:
        t = time.time()
        with torch.no_grad():
          episode_count, episode_reward = self.eval_unroll(self.agent, env, episode_length)
      ...
      for _ in range(num_epochs):
        observation, td = self.train_unroll(self.agent, env, observation, num_unrolls,
                                     unroll_length)
        ...
        td = self.sd_map(unroll_first, td)

        # update normalization statistics
        self.agent.update_normalization(td.observation)
        ...
        for _ in range(num_update_epochs):
          ...
          with torch.no_grad():
            ...
            epoch_td = self.sd_map(shuffle_batch, td)

          for minibatch_i in range(num_minibatches):
            td_minibatch = self.sd_map(lambda d: d[minibatch_i], epoch_td)
  ...
  def save(self, file_path, verbose=True):
    if(verbose): print("[INFO] Saving checkpoint:", file_path)
    policy_dict = self.agent.policy.state_dict()
    value_dict = self.agent.value.state_dict()
    optimizer_state_dict = self.optimizer.state_dict()
    torch.save({"policy": policy_dict,
                "value": value_dict,
